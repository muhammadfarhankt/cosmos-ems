...
HTTPS_GIT := github.com/rollchains/hackmoschain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=hackmoschain \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=hackmosd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/hackmosd ./cmd/hackmosd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/hackmosd.exe ./cmd/hackmosd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/hackmosd
...
draw-deps:
	...
	@goviz -i ./cmd/hackmosd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c hackmoschain --local -f chains.yaml
endif
...
set-testnet-configs:
	hackmosd config set client chain-id localchain-1
	hackmosd config set client keyring-backend test
	hackmosd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | hackmosd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | hackmosd keys add acc1 --recover`
