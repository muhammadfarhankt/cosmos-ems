...
# then log output of file /code/bin/hackmosd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/hackmosd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/hackmosd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/hackmosd /usr/bin/hackmosd
